...
Only the current pre-release version of [`ash`][ash] (the fundamental Vulkan bindings used by most rust projects) work on the Mac so I've attempted to update `skulpin` to work with that. A few releases ago, `skulpin` moved its internals to a much larger (and broader) framework called rafx which was too heavyweight for my purposes, so this directory instead contains a [fork](tree/main/vulkan/skulpin-renderer) of the [last pre-rafx version][skulpin_fork] of `skulpin`.
...
[gl]: tree/main/gl
[metal]: tree/main/metal
[vulkan]: tree/main/vulkan
